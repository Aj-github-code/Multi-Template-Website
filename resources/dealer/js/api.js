  ...
  return new Promise((resolve,reject)=>{
    var config = {
      ...
      axios(config).then(function (response) {
        ...
        if(response.success){
          localStorage.setItem('_token_aitechiez',  this.cryptCtrl.encrypt(response.access_token))
          localStorage.setItem('aitechiez_user_roles',  this.cryptCtrl.encrypt(JSON.stringify(decryptedData.user_roles)));
          localStorage.setItem('aitechiez_user_details', this.cryptCtrl.encrypt(JSON.stringify(decryptedData.user_details)));
          ...
          var config = {
            ...
            axios(config).then(function (response) {
              ...
              if(response.data.access_token){
                ...
                // timeObject = new Date(timeObject.getTime() + milliseconds);
                // localStorage.setItem('expire_token', timeObject);
                localStorage.setItem('_token_aitechiez',  this.cryptCtrl.encrypt(response.data.access_token))
              resolve(response.data.access_token);
              } else {
                resolve(this.login());
              }
   ...
   getToken(){
    return new Promise((resolve,reject)=>{
        if(localStorage.getItem('_token_aitechiez')){
          var token = null;
          token = this.cryptCtrl.decrypt(localStorage.getItem('_token_aitechiez'))
          if(token != null){
