...
login(){
  ...
  var encryptedData = cryptCtrl.encrypt(JSON.stringify(data));
  var config = {
    method: 'post',
    url: API_CONSTANTS.BASE_URL+'/login',
  ...
  axios(config).then(function (response) {
    ...
    if(response.data.status === 'success'){
      ...
      localStorage.setItem('_token_aitechiez',  cryptCtrl.encrypt(response.data.access_token))
      if(response.data.data){
        var decryptedData = JSON.parse(cryptCtrl.decrypt(response.data.data));
        localStorage.setItem('aitechiez_user_roles',  cryptCtrl.encrypt(JSON.stringify(decryptedData.user_roles)));
        localStorage.setItem('aitechiez_user_details', cryptCtrl.encrypt(JSON.stringify(decryptedData.user_details)));
  ...
  refreshToken(){
    ...
    const cryptCtrl = new Crypt;
      return new Promise((resolve,reject)=>{
          var config = {
              ...
              headers: { 
                  'Authorization': 'Bearer '+localStorage.getItem('_token_aitechiez')
              },
          };
            axios(config).then(function (response) {
              // console.log(response);
   ...
   getToken(){
    const cryptCtrl = new Crypt;
    return new Promise((resolve,reject)=>{
      // console.log("TOKEN", localStorage.getItem('_token_aitechiez'))
        if(localStorage.getItem('_token_aitechiez')){
          var token = null;
          token = cryptCtrl.decrypt(localStorage.getItem('_token_aitechiez'))
          if(token != null){
